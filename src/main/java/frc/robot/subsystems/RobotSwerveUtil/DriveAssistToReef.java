...
public class DriveAssistToReef {
	...
	public DriveAssistToReef(VisionSubsystemRobot2025 visionSubsystem) {
	...
	public ChassisSpeeds update(ChassisSpeeds chassisSpeeds, Rotation2d robotHeading, Boolean isLeft, Boolean isRight) {
		...
		if ((isLeft || isRight)) { // removed getTV
			Translation2d leftError = PoseTracker.getInstance().getPose().getTranslation().minus(
					ReefUtill.getReefFacePoint(ReefUtill.getFaceFromVision(), this.visionSubsystem).getPointLeft());
			Translation2d RightError = PoseTracker.getInstance().getPose().getTranslation().minus(
					ReefUtill.getReefFacePoint(ReefUtill.getFaceFromVision(), this.visionSubsystem).getPointRight());
			Translation2d finalPoint;
			if (isRight) {
				finalPoint = RightError;
				SmartDashboard.putNumber("FinalPoint getx", ReefUtill
						.getReefFacePoint(ReefUtill.getFaceFromVision(), this.visionSubsystem).getPointLeft().getX());
				SmartDashboard.putNumber("FinalPoint gety", ReefUtill
						.getReefFacePoint(ReefUtill.getFaceFromVision(), this.visionSubsystem).getPointLeft().getY());
			...
			if (Robot.s_Alliance == Alliance.Red) {
				toReturn = ChassisSpeeds.fromFieldRelativeSpeeds(
						m_filterForward.calculate(-finalPoint.getX()) * m_kpForward,
						m_filterSide.calculate(-finalPoint.getY()) * m_kpSide, 0, robotHeading);
			} else {
				toReturn = ChassisSpeeds.fromFieldRelativeSpeeds(
						m_filterForward.calculate(-finalPoint.getX()) * m_kpForward,
						m_filterSide.calculate(-finalPoint.getY()) * m_kpSide, 0,
						Rotation2d.fromDegrees(getAngleToreef(robotHeading)));
			}
		}
	...
	private double getAngleToreef(Rotation2d wantedAngle) {
		try {
			double currentHeading = PoseTracker.getInstance().getHeading().getDegrees();
