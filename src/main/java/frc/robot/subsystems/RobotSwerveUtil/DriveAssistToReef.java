...
public class DriveAssistToReef {    
	...
	public ChassisSpeeds update(ChassisSpeeds chassisSpeeds, Rotation2d robotHeading, Boolean isLeft, Boolean isRight){
		...
		if((isLeft || isRight)){ // removed getTV 
			Translation2d leftError = PoseTracker.getPose().getTranslation().minus(ReefUtill.getReefFacePoint(ReefUtill.getFaceFromVision(), this.visionSubsystem).getPointLeft());
			Translation2d RightError = PoseTracker.getPose().getTranslation().minus(ReefUtill.getReefFacePoint(ReefUtill.getFaceFromVision(), this.visionSubsystem).getPointRight());
	...
	private double getAngleToreef(Rotation2d wantedAngle) {
		try {
			double currentHeading = PoseTracker.getHeading().getDegrees();
