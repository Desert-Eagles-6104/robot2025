...
public class SwerveSubsystem extends SubsystemBase {
	...
	public static final double TRANSLATION_ERROR_MARGIN_FOR_RELEASING_PIECE_METERS_DRIVE_TO_POINT = Units
			.inchesToMeters(1.0);
	...
	public SwerveSubsystem(SwerveIOCTRE io, CommandPS5Controller controller, double maxVelocity,
			double maxAngularVelocity, SysIdRoutine.Config translationSysIdRoutineConfig,
	...
	public void periodic() {
		...
		this.io.refreshData();
		this.io.updateModuleInputs(this.frontLeftInputs, this.frontRightInputs, this.backLeftInputs,
				this.backRightInputs);
		...
		switch (this.systemState) {
			...
			case CHOREO_PATH: {
				if (this.choreoSampleToBeApplied.isPresent()) {
					...
					Logger.recordOutput("Subsystems/Drive/Choreo/Total time",
							this.desiredChoreoTrajectory.getTotalTime());
					Logger.recordOutput("Subsystems/Drive/Choreo/sample/Desired Pose", sample.getPose());
					Logger.recordOutput("Subsystems/Drive/Choreo/sample/Desired Chassis Speeds",
					...
					synchronized (this.swerveInputs) {
						var pose = this.swerveInputs.Pose;// is the same as geting the pose from this.io.getPose() but
															// will work using simultion
						...
						targetSpeeds.omegaRadiansPerSecond += this.choreoThetaController
			...
			case ROTATION_LOCK:
				ChassisSpeeds speeds = this.calculateSpeedsBasedOnJoystickInputs();
				...
				break;
			case DRIVE_TO_POINT:
				Translation2d translationToDesiredPoint = this.desiredPoseForDriveToPoint.getTranslation()
				...
				this.driveAtAngle.withVelocityX(xComponent)
						.withVelocityY(yComponent)
						.withTargetDirection(this.desiredPoseForDriveToPoint.getRotation());

				if (Double.isFinite(this.maximumAngularVelocityForDriveToPoint)) {
					this.driveAtAngle.withMaxAbsRotationalRate(this.maximumAngularVelocityForDriveToPoint);
				}
	...
	public void setDesiredPoseForDriveToPointWithMaximumAngularVelocity(Pose2d pose,
	...
	public void setDesiredPoseForDriveToPointWithConstraints(Pose2d pose, double maxVelocityOutputForDriveToPoint,
			double maximumAngularVelocityForDriveToPoint) {
		this.desiredPoseForDriveToPoint = pose;
		this.setWantedState(DriveState.DRIVE_TO_POINT);
		this.maxVelocityOutputForDriveToPoint = maxVelocityOutputForDriveToPoint;
