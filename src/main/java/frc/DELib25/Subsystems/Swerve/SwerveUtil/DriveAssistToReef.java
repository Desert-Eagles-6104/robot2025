...
public class DriveAssistToReef {    
    ...
    public DriveAssistToReef(double kp, BooleanSupplier intakeButton){
        m_kp = kp;
    ...
    public ChassisSpeeds update(ChassisSpeeds chassisSpeeds, Rotation2d robotHeading){
        ...
        if(VisionSubsystem.getTv() && m_intakeButton.getAsBoolean()){
            if(Robot.s_Alliance == Alliance.Red){
                toReturn = ChassisSpeeds.fromRobotRelativeSpeeds(0, -m_filter.calculate(-VisionSubsystem.getTx())*m_kp, 0, robotHeading);
            }
            else{
                toReturn = ChassisSpeeds.fromRobotRelativeSpeeds(0, m_filter.calculate(-VisionSubsystem.getTx())*m_kp, 0, robotHeading);
            }
